...
from sklearn.preprocessing import StandardScaler, Normalizer, MinMaxScaler, RobustScaler, MaxAbsScaler, QuantileTransformer, PowerTransformer
import matplotlib.pyplot as plt
from classifier import ClassifierNet, CONFIG as CLF_CONFIG
...
net = ClassifierNet(input_size, CLF_CONFIG["hidden_dims"])

if classifier == 'classifier_2':
    log("Loading best classifier_2 model for evaluation.", "INFO")
    best_state_dict = safe_load("weights/best_classifier_2.safetensors")
    load_state_dict(net, best_state_dict)
else:
    log("Loading best classifier model for evaluation.", "INFO")
    best_state_dict = safe_load("weights/best_classifier_model.safetensors")
    load_state_dict(net, best_state_dict)
...
def calculate_top_n_success(group_df):
    ...
    # Assuming higher score means higher confidence of being "good"
    group_df = group_df.sort_values('classifier_score', ascending=False)

    # Top-1 Pred-RMSD Success
    metrics['success_t1_pred_rmsd'] = n_poses>=1 and (group_df.iloc[0]['true_rmsd'] < RMSD_THRESHOLD)
    # Top-5 Pred-RMSD Success
    top5 = group_df.head(min(5, n_poses))
    metrics['success_t5_pred_rmsd'] = any(top5['true_rmsd'] < RMSD_THRESHOLD)
...
print(f"Number of unique pdb_ids processed: {results_df['pdb_id'].nunique()}")
